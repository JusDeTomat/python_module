    ...
    def process(self, data: Any) -> Union[str, Any]:
        ...
        try:
            if isinstance(data, str):
                ...
                rows = len(lines) - 1 if len(lines) > 1 else 1
            else:
                rows = 1
...
class StreamAdapter(ProcessingPipeline):
    ...
    def __init__(self, pipeline_id: str) -> None:
        """Initialize with 3 stages and empty readings list."""
        super().__init__(pipeline_id)
        self.add_stage(InputStage())
        self.add_stage(TransformStage())
        self.add_stage(OutputStage())
...
class NexusManager:
    ...
    def __init__(self) -> None:
        """Initialize with empty pipeline list and stats."""
    ...
    def process_data(self, data: Any) -> List[str]:
        """Send data through every registered pipeline polymorphically."""
        results: List[str] = []
    ...
    def chain_pipelines(
        ...
        for pipeline in targets:
            try:
                ...
                self._chain_log.append(f"{pipeline.pipeline_id}: OK")
            except Exception as e:
                self._chain_log.append(
                    f"{pipeline.pipeline_id}: ERROR {e}"
                )
        return str(current)
    ...
    def global_stats(self) -> Dict[str, Union[str, int]]:
        ...
        return {
            ...
            "total_processed": sum(
                p._processed_count for p in self.pipelines
            ),
